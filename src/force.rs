...
pub mod force_combat {
    ...
    pub fn run_round(force_a: Force, force_b: Force) -> (Force, Force, Vec<CombatLog>) {
        ...
        let (force_a_rtn, mut log_a) = calculate_losses(force_a.clone(), force_b.clone());
        let (force_b_rtn, mut log_b) = calculate_losses(force_a.clone(), force_b.clone());
    ...
    pub fn run_combat(force_a: Force, force_b: Force) {
        ...
        loop {
            println!("\nRound: {:?}", round_count);
            println!("{:?} {:#?}", force_a.name, f_a);
            println!("{:?} {:#?}", force_b.name, f_b);
